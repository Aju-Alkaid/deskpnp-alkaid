...
static void ST7306_HardReset(void) {
    RES_High; BusyDelay(10);
    RES_Low;  BusyDelay(20);
    RES_High; BusyDelay(120);
}
/*
 * 设置电压参数，影响对比度和稳定性
...
static void ST7306_InitSequence(void) {
    ...
    Write_Cmd(0x11); BusyDelay(120);// 重要：退出睡眠后需要足够的延时让电压稳定，至少 120ms，建议 200ms
...
//    Write_Cmd(0x39);                     
    BusyDelay(150);     // 重要：进入高功耗模式后需要足够的延时让电压稳定
    // 29h: Display On - 开启显示
...
void ST7306_Refresh(void) {
    ...
    CS_Low;
    DC_High;
    HAL_SPI_Transmit(hspi_lcd, display_buffer, LCD_BUFFER_SIZE, HAL_MAX_DELAY);
    CS_High;
}
