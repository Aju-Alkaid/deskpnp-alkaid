...
osThreadId_t PnPMotionTaskHandle; // 锟斤拷锟斤拷锟斤????
...
const osThreadAttr_t PnP_Motion_Task_attributes = { 
  .name = "PnPMotion",
  .stack_size = 256 * 4,  // 栈锟斤拷小锟斤拷锟街节ｏ????
  .priority = (osPriority_t)osPriorityNormal, // 锟斤拷锟饺硷????
};

//电机测试任务
const osThreadAttr_t motorTestTask_attributes = {
  .name = "MotorTest",
  .stack_size = 512,          // 堆栈给大一点，因为用了 printf
...
void StartUartTestTask(void *argument); // 閿熸枻鎷烽敓鏂ゆ嫹缂哄け閿熶茎鐚存嫹閿熸枻锟????
void StartMotorTestTask(void *argument); // 閿熸枻鎷烽敓鏂ゆ嫹缂哄け閿熶茎鐚存嫹閿熸枻锟????
void PnP_Motion_Task(void *argument);    // 閿熸枻鎷烽敓鏂ゆ嫹缂哄け閿熶茎鐚存嫹閿熸枻锟????
